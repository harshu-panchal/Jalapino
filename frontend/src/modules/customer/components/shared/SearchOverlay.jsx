...
import { Search, Mic, ArrowLeft, X, History, ShoppingBag, ChevronRight, Loader2 } from 'lucide-react';
...
const SearchOverlay = ({ isOpen, onClose }) => {
    ...
    useEffect(() => {
        ...
        fetchProducts();
    }, [isOpen, currentLocation?.latitude, currentLocation?.longitude, debouncedQuery]);
    ...
    return (
        <AnimatePresence>
            <motion.div
                ...
                className="fixed inset-0 z-[1000] bg-white flex flex-col font-sans overflow-hidden"
            >
                {/* Header / Search Input */}
                <div className={cn(
                    "sticky top-0 z-50 bg-linear-to-r from-primary to-[var(--brand-400)] shadow-[0_4px_20px_rgba(0,0,0,0.12)] overflow-hidden",
                ...
                {/* Search Content */}
                <div className="flex-1 overflow-y-auto p-5 space-y-10 pb-24">
                    {/* Search Results List */}
                    {query ? (
                        <section>
                        ...
                        <>
                            ...
                            <section>
                                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Lowest Prices</h3>
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-3 md:gap-x-4 gap-y-6 md:gap-y-10">
                                    {isLoading && allProducts.length === 0 ? (
                                        [...Array(6)].map((_, i) => (
                                            <div key={i} className="w-full h-52 md:h-64 bg-slate-50 rounded-2xl animate-pulse" />
                                        ))
                ...
                {/* Sticky View Cart Banner at the bottom of Search Overlay */}
                {cartCount > 0 && (
